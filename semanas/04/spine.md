...
**Through-line:** La working memory del agente es finita por diseño y no arranca vacía: el runtime carga instrucciones antes de tu primer mensaje y el loop la llena más rápido de lo que intuís. El **context rot** es la consecuencia (la señal se diluye en el ruido), el costo es la otra cara (el contexto se reenvía entero en cada vuelta, el cache lo abarata pero no lo elimina), y la respuesta operativa son dos movimientos: compactar o resetear.
**Hook:** Slide opener con la definición operativa (`alcance: una sesión completa`) y la tesis revelada por fragment.
**What students walk away knowing:**
- **La sesión no arranca vacía**: barra CSS con tres reveals (~15k al arranque con instrucciones del proyecto, reglas y memoria / ~500k sesión normal / ~900k sesión larga sobre 1M). Cierre: ventanas más grandes desplazan el límite, no lo eliminan; es finita por diseño.
- **Context rot**: diagrama de capas con la instrucción original (12 tokens) sepultada bajo observaciones de las vueltas 14, 28 y 35; en la vuelta 42 el agente contradice la instrucción. Los cuatro síntomas del supervisor entran como una línea de chips; cierre: no es que el modelo falló, la ventana trabaja en contra.
- **Qué se paga**: el contexto se reenvía entero en cada vuelta (50 vueltas al 90% ≈ 45M tokens de input). El cache abarata: leer el prefijo cuesta ~10% del precio de input, escribirlo ~25% más; un cambio al principio invalida el cache desde ahí. De ahí la regla: lo estable primero, lo que cambia al final.
...
**Animations / interactive:** CSS-only. **Una sola** barra de relleno, en §4.2 (5/50/90% desde los `data-fill` visibles, con listeners de `fragmentshown`/`fragmenthidden`/`slidechanged`). El diagrama de context rot ya no lleva barra propia.
**Slide budget actual:** 5 slides (opener + tesis → la sesión no arranca vacía → context rot + síntomas → costo y cache → compactar/resetear + cierre). Recorte del 2026-09-15: se eliminaron "Parámetros vs ventana", "Finita por diseño" (absorbida en la barra) y "Síntomas" (absorbida en context rot), y la segunda barra que repetía la animación.
