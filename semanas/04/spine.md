...
**What students walk away knowing:**
- **La sesión no arranca vacía, y se llena sola** (6 clicks sobre una sola barra CSS): ~15k al arranque (instrucciones del proyecto, reglas, memoria) / ~500k sesión normal / ~900k sesión larga sobre 1M → ventanas más grandes desplazan el límite, no lo eliminan → **el ejemplo del rot en la misma barra**: aparece `"no uses dependencias externas"` dentro del bloque de ~15k y un codo lleva de la ventana a una caja LLM cuya salida es `$ uv add requests pandas sqlalchemy` → se nombra **context rot** y entran los cuatro síntomas del supervisor.
- **Qué se paga**: el contexto se reenvía entero en cada vuelta (50 vueltas al 90% ≈ 45M tokens de input). El cache abarata: leer el prefijo cuesta ~10% del precio de input, escribirlo ~25% más; un cambio al principio invalida el cache desde ahí. Regla: lo estable primero, lo que cambia al final.
...
**Animations / interactive:** CSS-only. **Una sola** barra de relleno (5/50/90% desde los `data-fill` visibles, con listeners de `fragmentshown`/`fragmenthidden`/`slidechanged`); el codo hacia la caja LLM es CSS puro.
