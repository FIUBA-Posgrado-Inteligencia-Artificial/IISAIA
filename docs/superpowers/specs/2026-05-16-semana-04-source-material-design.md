...
- **Bridge desde S3:** la S3 cerró con *"la próxima clase tomamos este yaml y se lo damos a una IA local para que escriba la implementación"*. La S4 recoge ese hilo: **esa IA local es el agente de hoy.**
...
| # | Archivo | Núcleo | Palanca | Slides |
|---|---------|--------|---------|--------|
| 1 | `01-de-escribir-a-actuar.md` | Apertura. Bridge del yaml de S3. Contraste de paradigmas: sugerencia/autocompletado en línea **vs** agente que planifica y ejecuta en loop (sin marcas). Por qué dirigir algo autónomo exige un modelo mental nuevo. | — | 5-6 |
...
| 10 | `10-sub-agents.md` | Palanca de **aislamiento de contexto**: un sub-agent corre su propio loop con su propia ventana y devuelve solo el resultado → no infla el contexto del padre. Paralelización. | El contexto | 6-7 |
| 11 | `11-plan-mode-y-control.md` | Palanca de **control sobre el loop**: plan mode (pensar antes de actuar), permisos (gate antes de cada action). El supervisor decide cuánta autonomía. Hooks/MCP nombrados al pasar como "más palancas". | El loop | 5-6 |
...
Cada slide-guión sigue el formato del demo de S3: **qué hago / qué digo / qué tienen que mirar / plan B si no coopera**. Anclados a su sección y a la palanca que ilustran.
...
| D7 | **Plan mode como freno** — misma tarea sin plan mode (actúa y se equivoca) vs. con plan mode (propone, se corrige antes de tocar un archivo). | §11 | Palanca de control sobre el loop. |
| D8 | **Path-scoped rule aparece/desaparece** — rule con `paths: src/api/**`; `/context` sin ella; tocar archivo fuera del glob (ausente); tocar archivo que matchea → entra. | §8 | La palanca de contexto más limpia: contexto condicional. |
...
- Hooks y MCP en profundidad (solo nombrados como "más palancas" en §11).
...
4. Puede ubicar cada mecanismo de Claude Code (CLAUDE.md, rules, skills, sub-agents, slash commands, plan mode) como **palanca sobre el loop o sobre el contexto**, y distinguir *siempre cargado* vs *condicional* vs *autocurado*.
